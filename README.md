...
1. Ensure your local PostgreSQL server is running (e.g., `sudo systemctl start postgresql` on Linux, `brew services start postgresql` on macOS, or via the **Services** app / `net start postgresql` on Windows).
